...
export const fr = {
  ...
  plans: {
    ...
    documentHint: 'Un texte, Word, PowerPoint, OpenDocument, EPUB ou PDF : chaque page, titre ou ligne qui porte une référence devient un jour.',
    ...
    documentSplitPage: 'Une page = un jour (le PDF est gardé)',
    ...
    documentPagesPerDay: 'Pages par jour',
    documentPageHint: 'Le PDF est conservé dans ton espace et se lit tel quel, page par page. Une page sans référence rejoint le jour suivant.',
    documentPagesJoined: (n: number) => `${n} page${n > 1 ? 's' : ''} sans référence, rattachée${n > 1 ? 's' : ''} au jour voisin`,
    documentSectionsJoined: (n: number) => `${n} section${n > 1 ? 's' : ''} sans référence, rattachée${n > 1 ? 's' : ''} au jour voisin`,
    documentUploadError: 'Le document n’a pas pu être déposé. Vérifie la connexion, puis réessaie.',
    duration: 'Durée',
    durations: {
      '1-year': '1 an',
      '6-months': '6 mois',
  ...
  planDetail: {
    ...
    showDocument: 'Lire le document du jour',
    page: (n: number) => `p. ${n}`,

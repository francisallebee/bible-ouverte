...
export const en: Dictionary = {
  ...
  newReading: {
    ...
    leaveTitle: 'Reading not saved',
    leaveWarning:
      'Your reading is not saved yet. You can save it now, or leave this '
      + 'page and lose it.',
    leaveSave: 'Save, then continue',
  ...
  settings: {
    ...
    goalMinutesHint: (mots: number) =>
      `Minutes are estimated from the passage’s word count, at ${mots} words per minute. Nothing is timed.`,
  ...
  notifications: {
    ...
    triggers: {
      daily: {
        ...
        hint: 'A PDF, EPUB, Word, OpenDocument or HTML file, kept and read as it is, day after day — no references added. You choose what each day reads.',
      },
      'plan-late': {
  ...
  plans: {
    ...
    documentHint: 'A text, Word, PowerPoint, OpenDocument, EPUB or PDF file: every line or heading carrying a reference becomes a day. Nothing of the file is kept.',
    documentChoose: 'Choose the document',
    documentReading: 'Reading the document…',
    documentPdf: (i: number, n: number) => `Reading the PDF, page ${i} of ${n}…`,
    documentDays: (n: number) => `${n} reading day${n !== 1 ? 's' : ''}`,
    documentIgnored: (n: number) => `${n} line${n !== 1 ? 's' : ''} without a reference, skipped`,
    documentEmpty: 'No Bible reference found in this document.',
    documentMore: (n: number) => `… and ${n} more`,
    documentSplit: 'Split',
    documentSplitHeading: 'One heading = one day',
    documentSplitLine: 'One line = one day',
    documentSplitPassage: 'One passage = one day',
    documentRhythm: 'Pace',
    ...
    lecture: {
      ...
      hint: 'A PDF, kept and read as it is, day after day — no references added. You choose what each day reads.',
      choose: 'Choose the document',
      ofPages: (n: number) => `${n} page${n !== 1 ? 's' : ''}`,
      ofSections: (n: number) => `${n} section${n !== 1 ? 's' : ''}`,
      days: 'Days',
      errors: {
        format: 'Only PDF, EPUB, Word (.docx), OpenDocument (.odt) and HTML can be read as they are.',
        taille: 'The file is over 20 MB.',
        illisible: 'This PDF could not be read.',
        depot: 'The document could not be uploaded. Check the connection, then try again.',
      },
      pages: {
        perDayLabel: 'Pages per day', perDay: 'pages per day', startAt: 'Start at page', endAt: 'End at page',
        of: (n: number) => `${n} page${n !== 1 ? 's' : ''}`, from: 'First page', to: 'Last page',
        range: (a: number, b: number) => (a === b ? `p. ${a}` : `pp. ${a}-${b}`),
      },
      sections: {
        perDayLabel: 'Sections per day', perDay: 'sections per day', startAt: 'Start at section', endAt: 'End at section',
        of: (n: number) => `${n} section${n !== 1 ? 's' : ''}`, from: 'First section', to: 'Last section',
        range: (a: number, b: number) => (a === b ? `section ${a}` : `sections ${a}-${b}`),
      },
      daysCount: 'Number of days',
      spread: 'Spread over that many days',
      byChapter: (n: number) => (n >= 2 ? `One chapter per day (${n})` : 'One chapter per day'),
      noChapters: 'This PDF has no bookmarks.',
      leftOut: (avant: number, apres: number) => [avant > 0 ? `${avant} before` : '', apres > 0 ? `${apres} after` : ''].filter(Boolean).join(', ') + ' left out',
      split: 'Split this day in two',
      merge: 'Merge with the next day',
      remove: 'Remove this day',
      addDay: 'Add a day',
    },
    documentSectionsJoined: (n: number) => `${n} section${n !== 1 ? 's' : ''} without a reference, attached to the neighbouring day`,
    duration: 'Duration',
    durations: {
      '1-year': '1 year',
      '6-months': '6 months',
  ...
  progress: {
    ...
    nextMilestone: (n: number) => `Next milestone: ${n} days`,
    allMilestones: 'All milestones reached',
    milestoneReached: (n: number) => `${n} days`,
    chaptersRead: 'Chapters started',
    chaptersWhole: 'Read in full',
    ...
    goalToday: (current: number, target: number, unite: string, periode: string) =>
      `${current} / ${target} ${unite === 'chapters' ? 'chapters' : unite === 'verses' ? 'verses' : 'minutes'} ${periode}`,
    oldTestament: 'Old Testament',
    newTestament: 'New Testament',
    chaptersOfTotal: (read: number, total: number) => `${read} / ${total} chapters`,
    enPourcentage: 'As percentages',
    ...
    badges: {
      ...
      'streak-3': { name: 'Regular', description: 'A 3-day streak' },
      'streak-7': { name: 'Persevering', description: 'A 7-day streak' },
      'streak-30': { name: 'Unstoppable', description: 'A 30-day streak' },
      'streak-100': { name: 'Living legend', description: 'A 100-day streak' },
  ...
  messages: {
    ...
    emptyHint: 'The Bible Ouverte team will write to you here if needed.',
    active: 'Active',
    archived: 'Archived',
    emptyArchived: 'No archived messages.',
    archive: 'Archive',
    unarchive: 'Unarchive',
    confirmDelete: 'Remove this message from your inbox?',
    ...
    reply: 'Reply',
    writeTo: (nom: string) => `Write to ${nom}`,
    writeToSelection: (n: number) => `Write to the ${n} accounts shown`,
    confirmBulk: (n: number) => `Send this message to ${n} people?`,
    sentCount: (n: number) => `✓ Sent to ${n} ${n > 1 ? 'people' : 'person'}`,
    unread: 'Unread',
    errors: {
      corpsVide: 'The message is empty.',
      sujetTropLong: 'The subject is too long.',
      corpsTropLong: 'The message is too long.',
      sansDestinataire: 'No recipient.',
    } as Record<string, string>,
  },
  ...
  avance: {
    ...
    import: {
      ...
      fileHint: 'Text, csv, Word, Excel, PowerPoint, OpenDocument, EPUB, PDF, or an audio recording (mp3, m4a, wav…) transcribed on the spot by a small speech model — nothing leaves your device.',
      fileRefus: {
        'audio-vide': 'No speech recognised in this recording.',
        'audio-trop-long': 'Recording too long (30 minutes at most): split it into several files.',
        'kindle-chiffre': 'Kindle formats (mobi, azw) are encrypted by Amazon: convert the book to EPUB, or copy its text.',
        'format-inconnu': 'Unrecognised format.',
        'trop-gros': 'File too large (20 MB at most).',
        'illisible': 'Unreadable file.',
      },
      sessionDefaultFichier: (nom: string, date: string) => `Import ${nom} · ${date}`,
      photoButton: 'Photo',
      photoHint: 'The camera opens directly; each photo is read on your device and its text is added to the field — take another photo for the next page. The engine and language data download once.',
      ocrLoading: 'Getting ready to read…',
      ocrProgress: (n: number) => `Reading the photo… ${n}%`,
      ocrEmpty: 'No text recognised in this photo.',
      ocrError: 'The photo could not be read.',
      sessionDefaultPhoto: (date: string) => `Photo import · ${date}`,
      linkLabel: 'Address of an online document',
      linkPlaceholder: 'https://…',
      linkButton: 'Read the link',
      linkHint: 'A web page, a PDF, a Word or EPUB file online — the server fetches it without keeping it, and your device extracts the text.',
      linkLoading: 'Fetching the document…',
      linkRefus: {
        'adresse-invalide': 'Invalid address: it must start with http:// or https://.',
        'adresse-interne': 'This address points to an internal network; it is not allowed.',
        'inaccessible': 'The document could not be fetched.',
        'trop-gros': 'Document too large (4 MB at most this way).',
        'acces': 'Access denied.',
      },
      sessionDefaultLien: (nom: string, date: string) => `Import ${nom} · ${date}`,
      pdfProgress: (i: number, n: number) => `Reading the PDF, page ${i} of ${n}…`,
      pdfOcr: (i: number, n: number, pct: number) => `Page ${i} of ${n} is scanned, recognising… ${pct}%`,
      audioDecoding: 'Decoding the recording…',
      audioModel: (pct: number) => `Downloading the speech model… ${pct}%`,
      audioTranscribing: 'Transcribing — a few minutes for a long recording…',
      sessionDefaultAudio: (nom: string, date: string) => `Audio import ${nom} · ${date}`,
  ...
  admin: {
    ...
    noResult: 'No account matches',
    showing: (debut: number, fin: number, total: number) => `${debut}–${fin} of ${total}`,
    ...
    openFiche: 'Open profile',
    ficheNotFound: 'Account not found',
    ficheIdentity: 'Identity',
    ficheAccount: 'Account',
    ficheActivity: 'Activity',
    ficheReadings: 'Recent readings',
    fichePlans: 'Reading plans',
    ficheTickets: 'Support tickets',
    ficheNothing: 'Nothing to show',
    ficheEmailConfirmed: 'Email confirmed',
    ...
    ficheSessions: 'Game sessions',
    fichePlanDays: 'Plan days',
    ficheNotProvided: 'Not provided',
  ...
  planDetail: {
    ...
    readDocument: (titre: string) => `Read ${titre}`,
    editSplit: 'Edit the split',
    previousDay: 'Previous day',
    nextDay: 'Next day',
    alreadyRead: 'Read',
    page: (n: number) => `p. ${n}`,
    pages: (a: number, b: number) => (a === b ? `p. ${a}` : `pp. ${a}-${b}`),
    sections: (a: number, b: number) => (a === b ? `section ${a}` : `sections ${a}-${b}`),
    documentLoading: 'Downloading the document…',
    documentError: 'The document could not be read.',
    zoom: 'Zoom',
    zoomIn: 'Zoom in',
    zoomOut: 'Zoom out',
    marginsCropped: 'Margins cropped — click for the full page',
    marginsFull: 'Full page — click to crop the margins',
    pageDark: 'Dark page — click for the light page',
    pageLight: 'Light page — click for the dark page',
  ...
  memorisation: {
    ...
    choisirPassage: 'A passage of your choice',
    mettreEnApprentissage: 'Start learning it',
    texteIndisponible: 'The text of this passage is not on this device: enable its translation in the settings.',
    reviser: 'Review',

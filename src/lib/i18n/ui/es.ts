...
export const es: Dictionary = {
  ...
  history: {
    ...
    passageCount: (n: number) => `${n} ${n !== 1 ? 'pasajes' : 'pasaje'}`,
    andMore: (n: number) => `y ${n} más`,
    selectGroup: 'Seleccionar todas las lecturas de este registro',
  ...
  settings: {
    ...
    goalScope: 'Lo que se cuenta',
    goalMinutesHint: (mots: number) =>
      `Los minutos se estiman según el número de palabras del pasaje, a ${mots} palabras por minuto. No se cronometra nada.`,
    goalScopes: { toutes: 'Todas las lecturas', livre: 'Un libro', plan: 'Un plan de lectura' },
    goalScopeBook: 'Libro',
    goalScopePlan: 'Plan',
    goalScopeMissing: 'plan eliminado',
    goalScopeSummary: (nom: string) => ` — en ${nom}`,
    fontPreview:
  ...
  notifications: {
    ...
    triggers: {
      daily: {
        ...
        hint: 'Un PDF, EPUB, Word, OpenDocument o HTML, conservado y leído tal cual, día tras día — sin referencias añadidas. Tú eliges lo que se lee cada día.',
      },
      'plan-late': {
  ...
  plans: {
    ...
    document: 'Desde un documento',
    documentHint: 'Un texto, Word, PowerPoint, OpenDocument, EPUB o PDF: cada línea o título con una referencia se convierte en un día. Nada del archivo se conserva.',
    documentChoose: 'Elegir el documento',
    documentReading: 'Leyendo el documento…',
    documentPdf: (i: number, n: number) => `Leyendo el PDF, página ${i} de ${n}…`,
    documentDays: (n: number) => `${n} día${n !== 1 ? 's' : ''} de lectura`,
    documentIgnored: (n: number) => `${n} línea${n !== 1 ? 's' : ''} sin referencia, omitida${n !== 1 ? 's' : ''}`,
    documentEmpty: 'Ninguna referencia bíblica encontrada en este documento.',
    documentMore: (n: number) => `… y ${n} más`,
    documentSplit: 'División',
    documentSplitHeading: 'Un título = un día',
    documentSplitLine: 'Una línea = un día',
    documentSplitPassage: 'Un pasaje = un día',
    documentRhythm: 'Ritmo',
    ...
    lecture: {
      ...
      hint: 'Un PDF, conservado y leído tal cual, día tras día — sin referencias añadidas. Tú eliges lo que se lee cada día.',
      choose: 'Elegir el documento',
      ofPages: (n: number) => `${n} página${n !== 1 ? 's' : ''}`,
      ofSections: (n: number) => `${n} sección${n !== 1 ? 'es' : ''}`,
      days: 'Los días',
      errors: {
        format: 'Solo PDF, EPUB, Word (.docx), OpenDocument (.odt) y HTML pueden leerse tal cual.',
        taille: 'El archivo supera los 20 MB.',
        illisible: 'No se pudo leer este PDF.',
        depot: 'No se pudo subir el documento. Comprueba la conexión e inténtalo de nuevo.',
      },
      pages: {
        perDayLabel: 'Páginas por día', perDay: 'páginas por día', startAt: 'Empezar en la página', endAt: 'Terminar en la página',
        of: (n: number) => `${n} página${n !== 1 ? 's' : ''}`, from: 'Primera página', to: 'Última página',
        range: (a: number, b: number) => (a === b ? `p. ${a}` : `pp. ${a}-${b}`),
      },
      sections: {
        perDayLabel: 'Secciones por día', perDay: 'secciones por día', startAt: 'Empezar en la sección', endAt: 'Terminar en la sección',
        of: (n: number) => `${n} sección${n !== 1 ? 'es' : ''}`, from: 'Primera sección', to: 'Última sección',
        range: (a: number, b: number) => (a === b ? `sección ${a}` : `secciones ${a}-${b}`),
      },
      daysCount: 'Número de días',
      spread: 'Repartir en tantos días',
      byChapter: (n: number) => (n >= 2 ? `Un capítulo por día (${n})` : 'Un capítulo por día'),
      noChapters: 'Este PDF no tiene marcadores.',
      leftOut: (avant: number, apres: number) => [avant > 0 ? `${avant} antes` : '', apres > 0 ? `${apres} después` : ''].filter(Boolean).join(', ') + ' fuera del plan',
      split: 'Dividir este día en dos',
      merge: 'Unir con el día siguiente',
      remove: 'Quitar este día',
      addDay: 'Añadir un día',
    },
    documentSectionsJoined: (n: number) => `${n} sección${n !== 1 ? 'es' : ''} sin referencia, unida${n !== 1 ? 's' : ''} al día vecino`,
    duration: 'Duración',
    durations: {
      '1-year': '1 año',
      '6-months': '6 meses',
  ...
  progress: {
    ...
    goalToday: (current: number, target: number, unite: string, periode: string) =>
      `${current} / ${target} ${unite === 'chapters' ? 'capítulos' : unite === 'verses' ? 'versículos' : 'minutos'} ${periode}`,
    ...
    badges: {
      ...
      'streak-3': { name: 'Regular', description: 'Una racha de 3 días' },
      'streak-7': { name: 'Perseverante', description: 'Una racha de 7 días' },
      'streak-30': { name: 'Imparable', description: 'Una racha de 30 días' },
      'streak-100': { name: 'Leyenda viva', description: 'Una racha de 100 días' },
  ...
  donate: {
    ...
    freeText:
      'Las doce traducciones que ofrecemos son de dominio público: no cuestan '
  ...
  authScreens: {
    ...
    birthDate: 'Fecha de nacimiento',
    optional: ' (opcional)',
    discoverySource: '¿Cómo conociste Bible Ouverte?',
    discoveryPlaceholder: 'Prefiero no decirlo',
    discoverySources: {
      internet: 'Internet',
      reseaux: 'Redes sociales',
      connaissance: 'Un conocido',
      autre: 'Otro',
    } as Record<string, string>,
    creating: 'Creando…',
  ...
  messages: {
    ...
    empty: 'Ningún mensaje por ahora.',
    emptyHint: 'El equipo de Bible Ouverte te escribirá aquí si hace falta.',
    active: 'Activos',
    archived: 'Archivados',
    emptyArchived: 'Ningún mensaje archivado.',
    archive: 'Archivar',
    unarchive: 'Desarchivar',
    confirmDelete: '¿Quitar este mensaje de tu bandeja?',
    fromAdmin: 'Bible Ouverte',
    you: 'Tú',
    subject: 'Asunto',
    subjectPlaceholder: 'Opcional',
    body: 'Mensaje',
    bodyPlaceholder: 'Escribe tu mensaje…',
    send: 'Enviar',
    sending: 'Enviando…',
    sent: '✓ Mensaje enviado',
    sendFailed: 'El envío falló. Comprueba tu conexión e inténtalo de nuevo.',
    reply: 'Responder',
    writeTo: (nom: string) => `Escribir a ${nom}`,
    writeToSelection: (n: number) => `Escribir a las ${n} cuentas mostradas`,
    confirmBulk: (n: number) => `¿Enviar este mensaje a ${n} personas?`,
    sentCount: (n: number) => `✓ Enviado a ${n} persona${n !== 1 ? 's' : ''}`,
    unread: 'Sin leer',
    errors: {
      corpsVide: 'El mensaje está vacío.',
      sujetTropLong: 'El asunto es demasiado largo.',
      corpsTropLong: 'El mensaje es demasiado largo.',
      sansDestinataire: 'Ningún destinatario.',
    } as Record<string, string>,
  },
  ...
  avance: {
    ...
    import: {
      ...
      fileButton: 'Elegir un archivo',
      fileHint: 'Texto, csv, Word, Excel, PowerPoint, OpenDocument, EPUB, PDF, o una grabación de audio (mp3, m4a, wav…) transcrita en el dispositivo por un pequeño modelo de voz — nada sale del dispositivo.',
      fileRefus: {
        'audio-vide': 'Ninguna voz reconocida en esta grabación.',
        'audio-trop-long': 'Grabación demasiado larga (30 minutos como máximo): divídela en varios archivos.',
        'kindle-chiffre': 'Los formatos Kindle (mobi, azw) están cifrados por Amazon: convierte el libro a EPUB, o copia su texto.',
        'format-inconnu': 'Formato no reconocido.',
        'trop-gros': 'Archivo demasiado grande (20 MB como máximo).',
        'illisible': 'Archivo ilegible.',
      },
      sessionDefaultFichier: (nom: string, date: string) => `Importación ${nom} · ${date}`,
      photoButton: 'Foto',
      photoHint: 'La cámara se abre directamente; cada foto se lee en tu dispositivo y su texto se añade al campo — toma otra foto para la página siguiente. El motor y el diccionario se descargan una vez.',
      ocrLoading: 'Preparando la lectura…',
      ocrProgress: (n: number) => `Leyendo la foto… ${n} %`,
      ocrEmpty: 'Ningún texto reconocido en esta foto.',
      ocrError: 'No se pudo leer la foto.',
      sessionDefaultPhoto: (date: string) => `Importación de foto · ${date}`,
      linkLabel: 'Dirección de un documento en línea',
      linkPlaceholder: 'https://…',
      linkButton: 'Leer el enlace',
      linkHint: 'Una página web, un PDF, un archivo Word o EPUB en línea — el servidor lo busca sin guardarlo, y tu dispositivo extrae el texto.',
      linkLoading: 'Recuperando el documento…',
      linkRefus: {
        'adresse-invalide': 'Dirección no válida: debe empezar por http:// o https://.',
        'adresse-interne': 'Esta dirección apunta a una red interna; no está permitida.',
        'inaccessible': 'No se pudo recuperar el documento.',
        'trop-gros': 'Documento demasiado grande (4 MB como máximo por esta vía).',
        'acces': 'Acceso denegado.',
      },
      sessionDefaultLien: (nom: string, date: string) => `Importación ${nom} · ${date}`,
      pdfProgress: (i: number, n: number) => `Leyendo el PDF, página ${i} de ${n}…`,
      pdfOcr: (i: number, n: number, pct: number) => `Página ${i} de ${n} escaneada, reconociendo… ${pct} %`,
      audioDecoding: 'Decodificando la grabación…',
      audioModel: (pct: number) => `Descargando el modelo de voz… ${pct} %`,
      audioTranscribing: 'Transcribiendo — unos minutos para una grabación larga…',
      sessionDefaultAudio: (nom: string, date: string) => `Importación de audio ${nom} · ${date}`,
  ...
  admin: {
    ...
    tabJournal: 'Registro',
    acqSources: 'De dónde vienen las cuentas',
    acqMonths: 'Registros por mes',
    acqCities: 'Ciudades más representadas',
    acqUnknown: 'Sin indicar',
    acqNoCity: 'Ninguna ciudad indicada por ahora.',
    acqCount: (n: number, pourcent: number) => `${n} · ${pourcent} %`,
    journalEmpty: 'Ninguna acción registrada.',
    journalRecipients: (n: number) => `${n} persona${n !== 1 ? 's' : ''}`,
    journalActions: {
      promote: 'promovió a',
      demote: 'degradó a',
      suspend: 'suspendió a',
      reactivate: 'reactivó a',
      delete_account: 'eliminó la cuenta de',
      message: 'escribió a',
    } as Record<string, string>,

    manageUsers: 'Gestionar usuarios',
    usersTitle: 'Usuarios',
    usersSubtitle: (n: number) => `${n} cuenta${n !== 1 ? 's' : ''}`,
    backToAdmin: 'Volver a la administración',
    searchPlaceholder: 'Nombre, correo o ciudad…',
    exportCsv: 'Exportar en CSV',
    noResult: 'Ninguna cuenta coincide',
    showing: (debut: number, fin: number, total: number) => `${debut}–${fin} de ${total}`,
    pageOf: (page: number, pages: number) => `Página ${page} / ${pages}`,
    previous: 'Anterior',
    next: 'Siguiente',
    segments: {
      tous: 'Todos',
      ...
      suspendus: 'Suspendidos',
      admins: 'Administradores',
      incomplets: 'Perfil incompleto',
    } as Record<string, string>,
    ...
    openFiche: 'Abrir la ficha',
    ficheNotFound: 'Cuenta no encontrada',
    ficheIdentity: 'Identidad',
    ficheAccount: 'Cuenta',
    ficheActivity: 'Actividad',
    ficheReadings: 'Últimas lecturas',
    fichePlans: 'Planes de lectura',
    ficheTickets: 'Tickets de soporte',
    ficheNothing: 'Nada que mostrar',
    ficheEmailConfirmed: 'Correo confirmado',
    ...
    ficheSessions: 'Sesiones de juego',
    fichePlanDays: 'Días de plan',
    ficheNotProvided: 'Sin indicar',
  ...
  planDetail: {
    ...
    readDocument: (titre: string) => `Leer ${titre}`,
    editSplit: 'Modificar el reparto',
    previousDay: 'Día anterior',
    nextDay: 'Día siguiente',
    alreadyRead: 'Leído',
    page: (n: number) => `p. ${n}`,
    pages: (a: number, b: number) => (a === b ? `p. ${a}` : `pp. ${a}-${b}`),
    sections: (a: number, b: number) => (a === b ? `sección ${a}` : `secciones ${a}-${b}`),
    documentLoading: 'Descargando el documento…',
    documentError: 'No se pudo leer el documento.',
    zoom: 'Zoom',
    zoomIn: 'Ampliar',
    zoomOut: 'Reducir',
    marginsCropped: 'Márgenes recortados — pulsar para la página entera',
    marginsFull: 'Página entera — pulsar para recortar los márgenes',
    pageDark: 'Página oscura — pulsar para la página clara',
    pageLight: 'Página clara — pulsar para la página oscura',
    ...
    steps: {
      ...
      'nouvelle-lecture': {
        ...
        body:
          ...
          + 'versículos.',
        points: [
          'El texto aparece antes de validar: lees y luego guardas.',
      ...
      recherche: {
        ...
        body:
          ...
          + 'abrir otro sitio.',
        points: [
          'Doce traducciones libres de derechos en cinco lenguas, de 1611 a 1996.',
      ...
      quiz: {
        ...
        body:
          'Unas pocas preguntas sacadas de tus propias lecturas, para ver qué '
          + 'queda de ellas. Nada se califica, nada se compara con nadie.',
        points: [
          'Las preguntas salen de los pasajes que has registrado.',
          'Tres formas: el libro, el capítulo o la palabra que falta.',
        ],
      },
      'verset-du-jour': {
        title: 'El versículo del día',
        body:
          'Un versículo ofrecido cada día, el mismo para todos, que cambia a '
          + 'medianoche. Nada que ajustar: está ahí cuando abres la página.',
        points: [
          'Puedes guardarlo como lectura, con su contexto, de un solo gesto.',
          'No depende ni de tus planes ni de tu historial.',
        ],
      },
      memorisation: {
        title: 'La memorización',
        body:
          'Aprender un versículo de memoria recordándolo en intervalos que se '
          + 'espacian. Se ocultan más palabras en cada pasada lograda.',
        points: [
          'En la primera pasada no se oculta nada: lees el versículo entero.',
          'El repaso vuelve cuando sirve, y no cuando lo pides.',
          'Existe un entrenamiento libre, que no toca ningún plazo.',
        ],
      },
      reglages: {
        title: 'Los ajustes',
      ...
      messages: {
        title: 'Los mensajes',
        body:
          'Un hilo entre tú y la administración de la aplicación. Lo que se te '
          + 'escribe aquí también se te envía por correo.',
        points: [
          'Puedes responder desde esta página.',
          'Sin conexión queda vacía: es la única pantalla sin caché local.',
        ],
      },
      support: {
        title: 'El soporte',
  ...
  memorisation: {
    ...
    choisirPassage: 'Un pasaje a tu elección',
    mettreEnApprentissage: 'Empezar a aprenderlo',
    texteIndisponible: 'El texto de este pasaje no está en el dispositivo: activa su traducción en los ajustes.',
    reviser: 'Repasar',
    ...
    consigne: (reveles: number, masques: number) =>
      masques === 0
        ? 'Primer paso: lee este versículo completo. Se ocultarán palabras en las próximas revisiones.'
        : reveles === 0

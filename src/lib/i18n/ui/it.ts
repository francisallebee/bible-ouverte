...
export const it: Dictionary = {
  ...
  notifications: {
    ...
    triggers: {
      ...
      'plan-late': {
        ...
        hint: 'Un PDF, EPUB, Word, OpenDocument o HTML, conservato e letto così com’è, giorno dopo giorno — senza riferimenti aggiunti. Scegli tu cosa leggere ogni giorno.',
      },
      'support-reply': {
  ...
  plans: {
    ...
    documentHint: 'Un testo, Word, PowerPoint, OpenDocument, EPUB o PDF: ogni riga o titolo con un riferimento diventa un giorno. Nulla del file è conservato.',
    documentChoose: 'Scegli il documento',
    documentReading: 'Lettura del documento…',
    documentPdf: (i: number, n: number) => `Lettura del PDF, pagina ${i} di ${n}…`,
    documentDays: (n: number) => `${n} ${n === 1 ? 'giorno' : 'giorni'} di lettura`,
    documentIgnored: (n: number) => `${n} ${n === 1 ? 'riga senza riferimento, saltata' : 'righe senza riferimento, saltate'}`,
    documentEmpty: 'Nessun riferimento biblico trovato in questo documento.',
    documentMore: (n: number) => `… e ${n} ${n === 1 ? 'altro' : 'altri'}`,
    documentSplit: 'Suddivisione',
    documentSplitHeading: 'Un titolo = un giorno',
    documentSplitLine: 'Una riga = un giorno',
    documentSplitPassage: 'Un passo = un giorno',
    documentRhythm: 'Ritmo',
    documentDated: 'Con date, a partire dal',
    documentFreeRhythm: 'Libero, spuntato al mio ritmo',
    documentRejected: 'Non riconosciuto',
    lecture: {
      title: 'Leggere un documento',
      hint: 'Un PDF, conservato e letto così com’è, giorno dopo giorno — senza riferimenti aggiunti. Scegli tu cosa leggere ogni giorno.',
      choose: 'Scegli il documento',
      ofPages: (n: number) => `${n} pagin${n !== 1 ? 'e' : 'a'}`,
      ofSections: (n: number) => `${n} sezion${n !== 1 ? 'i' : 'e'}`,
      days: 'I giorni',
      errors: {
        format: 'Solo PDF, EPUB, Word (.docx), OpenDocument (.odt) e HTML si leggono così come sono.',
        taille: 'Il file supera i 20 MB.',
        illisible: 'Questo PDF non è leggibile.',
        depot: 'Il documento non è stato caricato. Controlla la connessione e riprova.',
      },
      pages: {
        perDayLabel: 'Pagine al giorno', perDay: 'pagine al giorno', startAt: 'Iniziare a pagina', endAt: 'Finire a pagina',
        of: (n: number) => `${n} pagin${n !== 1 ? 'e' : 'a'}`, from: 'Prima pagina', to: 'Ultima pagina',
        range: (a: number, b: number) => (a === b ? `p. ${a}` : `pp. ${a}-${b}`),
      },
      sections: {
        perDayLabel: 'Sezioni al giorno', perDay: 'sezioni al giorno', startAt: 'Iniziare alla sezione', endAt: 'Finire alla sezione',
        of: (n: number) => `${n} sezion${n !== 1 ? 'i' : 'e'}`, from: 'Prima sezione', to: 'Ultima sezione',
        range: (a: number, b: number) => (a === b ? `sezione ${a}` : `sezioni ${a}-${b}`),
      },
      daysCount: 'Numero di giorni',
      spread: 'Ripartire in altrettanti giorni',
      byChapter: (n: number) => (n >= 2 ? `Un capitolo al giorno (${n})` : 'Un capitolo al giorno'),
      noChapters: 'Questo PDF non ha segnalibri.',
      leftOut: (avant: number, apres: number) => [avant > 0 ? `${avant} prima` : '', apres > 0 ? `${apres} dopo` : ''].filter(Boolean).join(', ') + ' fuori dal piano',
      split: 'Dividere questo giorno in due',
      merge: 'Unire al giorno seguente',
      remove: 'Togliere questo giorno',
      addDay: 'Aggiungere un giorno',
    },
    documentSectionsJoined: (n: number) => `${n} ${n === 1 ? 'sezione senza riferimento, unita' : 'sezioni senza riferimento, unite'} al giorno vicino`,
    duration: 'Durata',
    durations: {
      '1-year': '1 anno',
      '6-months': '6 mesi',
  ...
  progress: {
    ...
    chaptersRead: 'Capitoli iniziati',
    chaptersWhole: 'Letti per intero',
  ...
  avance: {
    ...
    import: {
      ...
      fileButton: 'Scegli un file',
      fileHint: 'Testo, csv, Word, Excel, PowerPoint, OpenDocument, EPUB, PDF, o una registrazione audio (mp3, m4a, wav…) trascritta sul posto da un piccolo modello vocale — nulla lascia il dispositivo.',
      fileRefus: {
        'audio-vide': 'Nessuna voce riconosciuta in questa registrazione.',
        'audio-trop-long': 'Registrazione troppo lunga (30 minuti al massimo): dividila in più file.',
        'kindle-chiffre': 'I formati Kindle (mobi, azw) sono cifrati da Amazon: converti il libro in EPUB, o copia il suo testo.',
        'format-inconnu': 'Formato non riconosciuto.',
        'trop-gros': 'File troppo grande (20 MB al massimo).',
        'illisible': 'File illeggibile.',
      },
      sessionDefaultFichier: (nom: string, date: string) => `Importazione ${nom} · ${date}`,
      photoButton: 'Foto',
      photoHint: 'La fotocamera si apre direttamente; ogni foto è letta sul tuo dispositivo e il suo testo si aggiunge al campo — scatta un’altra foto per la pagina seguente. Il motore e il dizionario si scaricano una volta.',
      ocrLoading: 'Preparazione della lettura…',
      ocrProgress: (n: number) => `Lettura della foto… ${n} %`,
      ocrEmpty: 'Nessun testo riconosciuto in questa foto.',
      ocrError: 'Impossibile leggere la foto.',
      sessionDefaultPhoto: (date: string) => `Importazione da foto · ${date}`,
      linkLabel: 'Indirizzo di un documento in rete',
      linkPlaceholder: 'https://…',
      linkButton: 'Leggi il collegamento',
      linkHint: 'Una pagina web, un PDF, un file Word o EPUB in rete — il server lo recupera senza conservarlo, e il tuo dispositivo ne estrae il testo.',
      linkLoading: 'Recupero del documento…',
      linkRefus: {
        'adresse-invalide': 'Indirizzo non valido: deve iniziare con http:// o https://.',
        'adresse-interne': 'Questo indirizzo punta a una rete interna; non è ammesso.',
        'inaccessible': 'Impossibile recuperare il documento.',
        'trop-gros': 'Documento troppo grande (4 MB al massimo per questa via).',
        'acces': 'Accesso negato.',
      },
      sessionDefaultLien: (nom: string, date: string) => `Importazione ${nom} · ${date}`,
      pdfProgress: (i: number, n: number) => `Lettura del PDF, pagina ${i} di ${n}…`,
      pdfOcr: (i: number, n: number, pct: number) => `Pagina ${i} di ${n} scansionata, riconoscimento… ${pct} %`,
      audioDecoding: 'Decodifica della registrazione…',
      audioModel: (pct: number) => `Scaricamento del modello vocale… ${pct} %`,
      audioTranscribing: 'Trascrizione in corso — qualche minuto per una registrazione lunga…',
      sessionDefaultAudio: (nom: string, date: string) => `Importazione audio ${nom} · ${date}`,
  ...
  planDetail: {
    ...
    readDocument: (titre: string) => `Leggi ${titre}`,
    editSplit: 'Modificare la suddivisione',
    previousDay: 'Giorno precedente',
    nextDay: 'Giorno seguente',
    alreadyRead: 'Letto',
    page: (n: number) => `p. ${n}`,
    pages: (a: number, b: number) => (a === b ? `p. ${a}` : `pp. ${a}-${b}`),
    sections: (a: number, b: number) => (a === b ? `sezione ${a}` : `sezioni ${a}-${b}`),
    documentLoading: 'Scaricamento del documento…',
    documentError: 'Il documento non è leggibile.',
    zoom: 'Zoom',
    zoomIn: 'Ingrandisci',
    zoomOut: 'Riduci',
    marginsCropped: 'Margini ritagliati — premere per la pagina intera',
    marginsFull: 'Pagina intera — premere per ritagliare i margini',
    pageDark: 'Pagina scura — premere per la pagina chiara',
    pageLight: 'Pagina chiara — premere per la pagina scura',
  ...
  memorisation: {
    ...
    choisirPassage: 'Un passo a tua scelta',
    mettreEnApprentissage: 'Inizia a impararlo',
    texteIndisponible: 'Il testo di questo passo non è sul dispositivo: attiva la sua traduzione nelle impostazioni.',
    reviser: 'Ripassa',

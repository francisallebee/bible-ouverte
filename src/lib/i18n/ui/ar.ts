...
export const ar: Dictionary = {
  ...
  newReading: {
    ...
    leaveTitle: 'القراءة غير محفوظة',
    leaveWarning:
      'لم تُحفظ قراءتك بعد. يمكنك حفظها الآن، أو مغادرة هذه الصفحة وفقدانها.',
    leaveSave: 'احفظ ثم تابع',
    leaveDiscard: 'المغادرة دون حفظ',
    leaveStay: 'البقاء في الصفحة',
  ...
  settings: {
    ...
    homePageHint:
      'الشاشة التي تصل إليها عند فتح التطبيق. إذا أخفيت هذه الصفحة أدناه، '
      + 'فستعود إلى «قراءة جديدة».',
    pagesUnderSettings: 'مُدرَجة تحت الإعدادات، بترتيب ثابت:',
    pagesHint: 'اختر الصفحات التي تبقى في القائمة. الإعدادات والقراءة الجديدة متاحتان دائمًا.',
    pagesOrderHint:
      'رتّبها كما تشاء: تنقل الأسهم الصفحة درجة واحدة. تبقى الإعدادات والإدارة أسفل ملفك الشخصي.',
    pageUp: (nom: string) => `رفع ${nom}`,
    pageDown: (nom: string) => `خفض ${nom}`,
    pagesOrderReset: 'استعادة الترتيب الأصلي',
  ...
  notifications: {
    ...
    triggers: {
      daily: {
        label: 'تذكير يومي',
        hint: 'ملف PDF أو EPUB أو Word أو OpenDocument أو HTML يُحتفظ به ويُقرأ كما هو، يومًا بعد يوم — بلا مراجع مضافة. أنت تختار ما يُقرأ كل يوم.',
      },
      'plan-late': {
  ...
  plans: {
    ...
    lecture: {
      title: 'استيراد مستند',
      hint: 'ملف PDF أو EPUB أو Word أو OpenDocument أو HTML يُحتفظ به ويُقرأ كما هو، يومًا بعد يوم. أنت تختار ما يُقرأ كل يوم؛ ومراجعه الكتابية تُضاف إلى قراءاتك بلمسة.',
      choose: 'اختيار المستند',
      ofPages: (n: number) => `${n} ${safahat(n)}`,
      ofSections: (n: number) => `${n} ${aqsam(n)}`,
      days: 'الأيام',
      errors: {
        format: 'لا يُقرأ كما هو إلا PDF وEPUB وWord (.docx) وOpenDocument (.odt) وHTML.',
        taille: 'الملف يتجاوز 20 ميغابايت.',
        illisible: 'تعذّرت قراءة هذا الملف.',
        depot: 'تعذّر رفع المستند. تحقّق من الاتصال ثم أعد المحاولة.',
      },
      pages: {
        perDayLabel: 'صفحات في اليوم', perDay: 'صفحات في اليوم', startAt: 'البدء من الصفحة', endAt: 'الانتهاء عند الصفحة',
        of: (n: number) => `${n} ${safahat(n)}`, from: 'الصفحة الأولى', to: 'الصفحة الأخيرة',
        range: (a: number, b: number) => (a === b ? `ص ${a}` : `ص ${a}-${b}`),
      },
      sections: {
        perDayLabel: 'أقسام في اليوم', perDay: 'أقسام في اليوم', startAt: 'البدء من القسم', endAt: 'الانتهاء عند القسم',
        of: (n: number) => `${n} ${aqsam(n)}`, from: 'القسم الأول', to: 'القسم الأخير',
        range: (a: number, b: number) => (a === b ? `القسم ${a}` : `الأقسام ${a}-${b}`),
      },
      rightsTitle: 'حقوق هذا المستند',
      rightsText: 'سيُحتفظ بهذا المستند في مساحتك الشخصية ولن يقرأه غيرك. أكّد أن لك الحق فيه: ملف اشتريته أو تلقيته لاستعمالك، أو خالٍ من الحقوق. لا يجوز استيراد محتوى لا تملك حقوقه.',
      rightsConfirm: 'أؤكد أن لي الحقوق',
      daysCount: 'عدد الأيام',
      spread: 'توزيع على هذا العدد من الأيام',
      byChapter: (n: number) => (n >= 2 ? `فصل واحد في اليوم (${n})` : 'فصل واحد في اليوم'),
      noChapters: 'هذا الملف بلا علامات مرجعية.',
      leftOut: (avant: number, apres: number) => [avant > 0 ? `${avant} قبل` : '', apres > 0 ? `${apres} بعد` : ''].filter(Boolean).join('، ') + ' خارج الخطة',
      split: 'شطر هذا اليوم إلى يومين',
      merge: 'ضمّ إلى اليوم التالي',
      remove: 'إزالة هذا اليوم',
      addDay: 'إضافة يوم',
    },
    duration: 'المدة',
    durations: {
  ...
  progress: {
    ...
    chaptersRead: 'إصحاحات بُدئت',
    chaptersWhole: 'مقروءة بالكامل',
  ...
  donate: {
    ...
    freeText:
      'الترجمات الاثنتا عشرة المتاحة في الملك العام: لا تكلّف شيئًا ولن تكلّف شيئًا '
  ...
  messages: {
    ...
    active: 'النشطة',
    archived: 'المؤرشفة',
    emptyArchived: 'لا توجد رسائل مؤرشفة.',
    archive: 'أرشفة',
    unarchive: 'إلغاء الأرشفة',
    confirmDelete: 'هل تريد إزالة هذه الرسالة من صندوقك؟',
  ...
  avance: {
    title: 'الوظائف المتقدمة',
    subtitle: 'منصة تجربة الوظائف القادمة',
    empty: 'لا شيء قيد التجربة حاليًا.',
    emptyHint:
    ...
    import: {
      ...
      fileHint: 'نص، csv، Word، Excel، PowerPoint، OpenDocument، EPUB، PDF، أو تسجيل صوتي (mp3، m4a، wav…) يُنسخ على الجهاز بنموذج كلام صغير — لا شيء يغادر جهازك.',
      fileRefus: {
        'audio-vide': 'لم يُعرف أي كلام في هذا التسجيل.',
        'audio-trop-long': 'التسجيل طويل جدًا (30 دقيقة على الأكثر): قسّمه إلى عدة ملفات.',
        'kindle-chiffre': 'صيغ Kindle (mobi، azw) مشفّرة من أمازون: حوّل الكتاب إلى EPUB، أو انسخ نصه.',
        'format-inconnu': 'صيغة غير معروفة.',
        'trop-gros': 'الملف كبير جدًا (20 ميغابايت على الأكثر).',
        'illisible': 'ملف غير قابل للقراءة.',
      },
      sessionDefaultFichier: (nom: string, date: string) => `استيراد ${nom} · ${date}`,
      photoButton: 'صورة',
      photoHint: 'تُفتح الكاميرا مباشرة؛ تُقرأ كل صورة على جهازك ويُضاف نصها إلى الحقل — التقط صورة أخرى للصفحة التالية. يُنزَّل المحرك والقاموس مرة واحدة.',
      ocrLoading: 'جارٍ التحضير للقراءة…',
      ocrProgress: (n: number) => `جارٍ قراءة الصورة… ${n}٪`,
      ocrEmpty: 'لم يُعرف أي نص في هذه الصورة.',
      ocrError: 'تعذّرت قراءة الصورة.',
      sessionDefaultPhoto: (date: string) => `استيراد من صورة · ${date}`,
      linkLabel: 'عنوان مستند على الإنترنت',
      linkPlaceholder: 'https://…',
      linkButton: 'قراءة الرابط',
      linkHint: 'صفحة ويب، أو PDF، أو ملف Word أو EPUB على الإنترنت — يجلبه الخادم دون الاحتفاظ به، ويستخرج جهازك النص.',
      linkLoading: 'جارٍ جلب المستند…',
      linkRefus: {
        'adresse-invalide': 'عنوان غير صالح: يجب أن يبدأ بـ http:// أو https://.',
        'adresse-interne': 'يشير هذا العنوان إلى شبكة داخلية؛ غير مسموح به.',
        'inaccessible': 'تعذّر جلب المستند.',
        'trop-gros': 'المستند كبير جدًا (4 ميغابايت على الأكثر بهذه الطريقة).',
        'acces': 'تم رفض الوصول.',
      },
      sessionDefaultLien: (nom: string, date: string) => `استيراد ${nom} · ${date}`,
      pdfProgress: (i: number, n: number) => `جارٍ قراءة PDF، الصفحة ${i} من ${n}…`,
      pdfOcr: (i: number, n: number, pct: number) => `الصفحة ${i} من ${n} ممسوحة، جارٍ التعرف… ${pct}٪`,
      audioDecoding: 'جارٍ فك ترميز التسجيل…',
      audioModel: (pct: number) => `جارٍ تنزيل نموذج الكلام… ${pct}٪`,
      audioTranscribing: 'جارٍ النسخ — بضع دقائق لتسجيل طويل…',
      sessionDefaultAudio: (nom: string, date: string) => `استيراد صوتي ${nom} · ${date}`,
  ...
  planDetail: {
    ...
    readDocument: (titre: string) => `قراءة ${titre}`,
    ...
    sections: (a: number, b: number) => (a === b ? `القسم ${a}` : `الأقسام ${a}-${b}`),
    documentLoading: 'جارٍ تنزيل المستند…',
    documentError: 'تعذّرت قراءة المستند.',
    zoom: 'تكبير',
    zoomIn: 'تكبير',
    zoomOut: 'تصغير',
    referenceFound: 'مرجع مرصود',
    addToReadings: 'إضافة إلى قراءاتي',
    referenceAdded: 'أُضيف إلى قراءاتك',
    referenceError: 'تعذّرت الإضافة.',
    reference: (l: string) => `المرجع: ${l}`,
    marginsCropped: 'الهوامش مقصوصة — اضغط للصفحة كاملة',
    marginsFull: 'الصفحة كاملة — اضغط لقصّ الهوامش',
    pageDark: 'صفحة داكنة — اضغط للصفحة الفاتحة',
    pageLight: 'صفحة فاتحة — اضغط للصفحة الداكنة',
  ...
  tour: {
    ...
    steps: {
      ...
      'nouvelle-lecture': {
        ...
        points: [
          'يظهر النص قبل التأكيد: تقرأ ثم تحفظ.',
      ...
      recherche: {
        ...
        points: [
          'اثنتا عشرة ترجمة حرة الحقوق بخمس لغات، من 1611 إلى 1996.',
      ...
      quiz: {
        title: 'الاختبار',
        body:
          'أسئلة قليلة مأخوذة من قراءاتك أنت، لترى ما بقي منها. لا شيء يُقيَّم، '
          + 'ولا شيء يُقارَن بأحد.',
        points: [
          'الأسئلة تأتي من المقاطع التي سجّلتها.',
          'ثلاثة أشكال: السفر، أو الإصحاح، أو الكلمة الناقصة.',
        ],
      },
      'verset-du-jour': {
        title: 'آية اليوم',
        body:
          'آية تُقدَّم كل يوم، هي نفسها للجميع، وتتغيّر عند منتصف الليل. لا شيء '
          + 'لضبطه: تجدها حين تفتح الصفحة.',
        points: [
          'يمكنك حفظها كقراءة، مع سياقها، بحركة واحدة.',
          'لا تتوقف على خططك ولا على سجلّك.',
        ],
      },
      memorisation: {
        title: 'الحفظ',
        body:
          'أن تحفظ آية عن ظهر قلب باستعادتها على فترات تتباعد. وتُخفى كلمات '
          + 'أكثر في كل جولة ناجحة.',
        points: [
          'في الجولة الأولى لا يُخفى شيء: تقرأ الآية كاملة.',
          'تعود المراجعة حين تنفع، لا حين تطلبها.',
          'وثمة تدريب حر لا يمسّ أي موعد.',
        ],
      },
      reglages: {
        title: 'الإعدادات',
      ...
      messages: {
        title: 'الرسائل',
        body:
          'خيط بينك وبين إدارة التطبيق. وما يُكتب إليك هنا يصلك بالبريد '
          + 'الإلكتروني أيضًا.',
        points: [
          'يمكنك الرد من هذه الصفحة.',
          'دون اتصال تبقى فارغة: هي الشاشة الوحيدة بلا ذاكرة محلية.',
        ],
      },
      support: {
        title: 'الدعم',
      ...
      avance: {
        title: 'الوظائف المتقدمة',
        body:
          'منصة التجربة: تظهر الوظائف هنا قبل أن تُتاح للجميع. مخصَّصة لحسابك، '
          + 'وتبقى فارغة ما دام لا شيء قيد التجربة.',
        points: [],
      },
      fin: {
        title: 'الدور عليك',
  ...
  memorisation: {
    ...
    choisir: 'من قراءاتك',
    choisirPassage: 'مقطع من اختيارك',
    mettreEnApprentissage: 'ابدأ حفظه',
    texteIndisponible: 'نص هذا المقطع غير موجود على الجهاز: فعّل ترجمته في الإعدادات.',
    reviser: 'راجع',

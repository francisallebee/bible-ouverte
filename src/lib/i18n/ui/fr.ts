...
export const fr = {
  ...
  notifications: {
    ...
    triggers: {
      ...
      inactive: {
        ...
        hint: 'Un PDF, un EPUB, un Word, un OpenDocument ou un HTML, gardé et lu tel qu’il est, jour après jour — sans référence ajoutée. Tu choisis ce que chaque jour lit.',
      },
      birthday: {
        label: 'Anniversaire',
  ...
  plans: {
    ...
    document: 'Depuis un document',
    documentHint: 'Un texte, Word, PowerPoint, OpenDocument, EPUB ou PDF : chaque ligne ou titre qui porte une référence devient un jour. Rien du fichier n’est gardé.',
    documentChoose: 'Choisir le document',
    documentReading: 'Lecture du document…',
    documentPdf: (i: number, n: number) => `Lecture du PDF, page ${i} sur ${n}…`,
    documentDays: (n: number) => `${n} jour${n > 1 ? 's' : ''} de lecture`,
    documentIgnored: (n: number) => `${n} ligne${n > 1 ? 's' : ''} sans référence, passée${n > 1 ? 's' : ''}`,
    documentEmpty: 'Aucune référence biblique trouvée dans ce document.',
    documentMore: (n: number) => `… et ${n} autre${n > 1 ? 's' : ''}`,
    documentSplit: 'Découpage',
    documentSplitHeading: 'Un titre = un jour',
    documentSplitLine: 'Une ligne = un jour',
    documentSplitPassage: 'Un passage = un jour',
    documentRhythm: 'Rythme',
    ...
    lecture: {
      ...
      hint: 'Un PDF, gardé et lu tel qu’il est, jour après jour — sans référence ajoutée. Tu choisis ce que chaque jour lit.',
      choose: 'Choisir le document',
      ofPages: (n: number) => `${n} page${n > 1 ? 's' : ''}`,
      ofSections: (n: number) => `${n} section${n > 1 ? 's' : ''}`,
      days: 'Les jours',
      errors: {
        format: 'Seuls PDF, EPUB, Word (.docx), OpenDocument (.odt) et HTML se lisent tels quels.',
        taille: 'Le fichier dépasse 20 Mo.',
        illisible: 'Ce PDF n’a pas pu être lu.',
        depot: 'Le document n’a pas pu être déposé. Vérifie la connexion, puis réessaie.',
      },
      pages: {
        perDayLabel: 'Pages par jour', perDay: 'pages par jour', startAt: 'Commencer à la page', endAt: 'Finir à la page',
        of: (n: number) => `${n} page${n > 1 ? 's' : ''}`, from: 'Première page', to: 'Dernière page',
        range: (a: number, b: number) => (a === b ? `p. ${a}` : `p. ${a}-${b}`),
      },
      sections: {
        perDayLabel: 'Sections par jour', perDay: 'sections par jour', startAt: 'Commencer à la section', endAt: 'Finir à la section',
        of: (n: number) => `${n} section${n > 1 ? 's' : ''}`, from: 'Première section', to: 'Dernière section',
        range: (a: number, b: number) => (a === b ? `section ${a}` : `sections ${a}-${b}`),
      },
      daysCount: 'Nombre de jours',
      spread: 'Répartir en autant de jours',
      byChapter: (n: number) => (n >= 2 ? `Un chapitre par jour (${n})` : 'Un chapitre par jour'),
      noChapters: 'Ce PDF n’a pas de signets.',
      leftOut: (avant: number, apres: number) => [avant > 0 ? `${avant} avant` : '', apres > 0 ? `${apres} après` : ''].filter(Boolean).join(', ') + ' hors du plan',
      split: 'Scinder ce jour en deux',
      merge: 'Fusionner avec le jour suivant',
      remove: 'Retirer ce jour',
      addDay: 'Ajouter un jour',
    },
    documentSectionsJoined: (n: number) => `${n} section${n > 1 ? 's' : ''} sans référence, rattachée${n > 1 ? 's' : ''} au jour voisin`,
    duration: 'Durée',
    durations: {
      '1-year': '1 an',
      '6-months': '6 mois',
  ...
  progress: {
    ...
    chaptersRead: 'Chapitres entamés',
    chaptersWhole: 'Lus en entier',
  ...
  messages: {
    ...
    empty: 'Aucun message pour le moment.',
    emptyHint: 'L’équipe de Bible Ouverte t’écrira ici si besoin.',
    active: 'Actifs',
    archived: 'Archivés',
    emptyArchived: 'Aucun message archivé.',
    archive: 'Archiver',
    unarchive: 'Désarchiver',
    confirmDelete: 'Retirer ce message de ta boîte ?',
    fromAdmin: 'Bible Ouverte',
    you: 'Toi',
    subject: 'Sujet',
    subjectPlaceholder: 'Facultatif',
    body: 'Message',
    bodyPlaceholder: 'Écris ton message…',
    send: 'Envoyer',
    sending: 'Envoi…',
    sent: '✓ Message envoyé',
    sendFailed: 'L’envoi a échoué. Vérifie ta connexion et réessaie.',
    reply: 'Répondre',
    writeTo: (nom: string) => `Écrire à ${nom}`,
    writeToSelection: (n: number) => `Écrire aux ${n} comptes affichés`,
    confirmBulk: (n: number) => `Envoyer ce message à ${n} personnes ?`,
    sentCount: (n: number) => `✓ Envoyé à ${n} personne${n > 1 ? 's' : ''}`,
    unread: 'Non lu',
    errors: {
      corpsVide: 'Le message est vide.',
      sujetTropLong: 'Le sujet est trop long.',
      corpsTropLong: 'Le message est trop long.',
      sansDestinataire: 'Aucun destinataire.',
    } as Record<string, string>,
  },
  ...
  avance: {
    ...
    import: {
      ...
      fileHint: 'Texte, csv, Word, Excel, PowerPoint, OpenDocument, EPUB, PDF, ou un enregistrement audio (mp3, m4a, wav…) transcrit sur place par un petit modèle de parole — rien ne quitte l’appareil.',
      fileRefus: {
        'audio-vide': 'Aucune parole reconnue dans cet enregistrement.',
        'audio-trop-long': 'Enregistrement trop long (30 minutes au plus) : coupe-le en plusieurs fichiers.',
        'kindle-chiffre': 'Les formats Kindle (mobi, azw) sont chiffrés par Amazon : convertis le livre en EPUB, ou copie son texte.',
        'format-inconnu': 'Format non reconnu.',
        'trop-gros': 'Fichier trop volumineux (20 Mo au plus).',
        'illisible': 'Fichier illisible.',
      },
      sessionDefaultFichier: (nom: string, date: string) => `Import ${nom} · ${date}`,
      photoButton: 'Photo',
      photoHint: 'L’appareil s’ouvre directement ; chaque photo est lue sur ton appareil et son texte s’ajoute au champ — reprends une photo pour la page suivante. Le moteur et le dictionnaire se téléchargent une fois.',
      ocrLoading: 'Préparation de la lecture…',
      ocrProgress: (n: number) => `Lecture de la photo… ${n} %`,
      ocrEmpty: 'Aucun texte reconnu sur cette photo.',
      ocrError: 'La photo n’a pas pu être lue.',
      sessionDefaultPhoto: (date: string) => `Import photo · ${date}`,
      linkLabel: 'Adresse d’un document en ligne',
      linkPlaceholder: 'https://…',
      linkButton: 'Lire le lien',
      linkHint: 'Une page web, un PDF, un fichier Word ou EPUB en ligne — le serveur va le chercher sans le garder, et ton appareil en extrait le texte.',
      linkLoading: 'Récupération du document…',
      linkRefus: {
        'adresse-invalide': 'Adresse invalide : elle doit commencer par http:// ou https://.',
        'adresse-interne': 'Cette adresse désigne un réseau interne ; elle n’est pas admise.',
        'inaccessible': 'Le document n’a pas pu être récupéré.',
        'trop-gros': 'Document trop volumineux (4 Mo au plus par ce chemin).',
        'acces': 'Accès refusé.',
      },
      sessionDefaultLien: (nom: string, date: string) => `Import ${nom} · ${date}`,
      pdfProgress: (i: number, n: number) => `Lecture du PDF, page ${i} sur ${n}…`,
      pdfOcr: (i: number, n: number, pct: number) => `Page ${i} sur ${n} scannée, reconnaissance… ${pct} %`,
      audioDecoding: 'Décodage de l’enregistrement…',
      audioModel: (pct: number) => `Téléchargement du modèle de parole… ${pct} %`,
      audioTranscribing: 'Transcription en cours — quelques minutes pour un long enregistrement…',
      sessionDefaultAudio: (nom: string, date: string) => `Import audio ${nom} · ${date}`,
  ...
  admin: {
    ...
    noResult: 'Aucun compte ne correspond',
    showing: (debut: number, fin: number, total: number) => `${debut}–${fin} sur ${total}`,
    ...
    ficheNotFound: 'Compte introuvable',
    ficheIdentity: 'Identité',
    ficheAccount: 'Compte',
    ficheActivity: 'Activité',
    ficheReadings: 'Dernières lectures',
    fichePlans: 'Plans de lecture',
    ficheTickets: 'Tickets support',
    ficheNothing: 'Rien à afficher',
    ficheEmailConfirmed: 'Adresse confirmée',
    ...
    ficheSessions: 'Séances de jeu',
    fichePlanDays: 'Jours de plan',
    ficheNotProvided: 'Non renseigné',
  ...
  planDetail: {
    ...
    readDocument: (titre: string) => `Lire ${titre}`,
    editSplit: 'Modifier le découpage',
    previousDay: 'Jour précédent',
    nextDay: 'Jour suivant',
    alreadyRead: 'Lu',
    page: (n: number) => `p. ${n}`,
    pages: (a: number, b: number) => (a === b ? `p. ${a}` : `p. ${a}-${b}`),
    sections: (a: number, b: number) => (a === b ? `section ${a}` : `sections ${a}-${b}`),
    documentLoading: 'Téléchargement du document…',
    documentError: 'Le document n’a pas pu être lu.',
    zoom: 'Zoom',
    zoomIn: 'Agrandir',
    zoomOut: 'Réduire',
    marginsCropped: 'Marges rognées — cliquer pour la page entière',
    marginsFull: 'Page entière — cliquer pour rogner les marges',
    pageDark: 'Page sombre — cliquer pour la page claire',
    pageLight: 'Page claire — cliquer pour la page sombre',
  ...
  memorisation: {
    ...
    choisirPassage: 'Un passage de ton choix',
    mettreEnApprentissage: 'Mettre en apprentissage',
    texteIndisponible: 'Le texte de ce passage n’est pas sur l’appareil : active sa traduction dans les réglages.',
    reviser: 'Réviser',
